...
These benchmarks check the equivalence of two circuits using a construction
called a \emph{miter}: given two circuits $C_1$ and $C_2$ with the same
input and output
...
formula.
They implemented their algorithm as a preprocessing pass in
Kissat~\cite{kissat} and evaluated on two benchmark sets: 10 hand-curated
miters from the IWLS'22 paper~\cite{iwls}, considered hard for monolithic
CNF-level SAT sweeping, and a derivative of the 2012 Hardware Model
Checking Competition~\cite{hwmcc} that compares each And-Inverter Graph
