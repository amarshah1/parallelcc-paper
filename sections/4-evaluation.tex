...
performance of our algorithm. Finally, in \autoref{sec:eval:hardware}, we discuss
...
Essentially, this setup divides the set of leaves into two We
...
from cache misses and other overhead. In particular, maintaining parent lists
within the sequential algorithm (and the parent-based parallel algorithm)
has relatively poor spatial locality, as the children of a node may be scattered
far away from the parent in memory.
...
Our benchmark suite generalizes the structure of the example in
\autoref{ex:formula} to arbitrary widths and depths. Each benchmark is
parameterized by a width $k$ and a $g$-tree fan-in $l$, and is constructed
in three layers:
...
equivalent subcircuits whose equivalence can be discovered structurally. They
showed that recovering the gates from the CNF and running congruence closure
on them can drastically simplify (and often immediately decide) the resulting
formula.
