...
\parentalgo maintains a parent list for each congruence class, and when
classes are merged, the parents of the merged classes are re-examined in the
next round. The second algorithm \filteralgo marks a class as dirty when it
is merged and then filters all terms to find those with a child in a dirty
class.
...
\begin{itemize}
    ...
    that, on our benchmarks, width is the dominant factor.
    \item Despite congruence closure being \textsc{P}-complete (i.e. believed to not
    ...
    \item We open source our code and benchmarks\footnote{Our code is available at \url{https://github.com/amarshah10/ParallelEgraph}}.
\end{itemize}
