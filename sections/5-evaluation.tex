...
All experiments were run on a 96-core Amazon Web Service \texttt{c8i-metal}
instance with Intel(R) Xeon(R) 6975P-C (3.9 GHz) and 384 GB memory. Each core is
2-way hyperthreaded, giving 192 hyperthreads. The machine runs Ubuntu 26.04
LTS, and the code was compiled using g++15.2.0 with -O3.
...
$n_{\text{fns}}$ total functions. Then we have a set of terms $T$ that
...
where each pair contains one \texttt{x}-leaf and one \texttt{y}-leaf. We
...
\begin{table}[t]
  ...
  \begin{tabular}{l r r r r r}
    ...
    \bottomrule
  \end{tabular}
  \caption{Synthetic workload parameters. We have $n_{\text{fns}}$ function
  symbols, each of arity $2$. Each workload is a random binary-tree DAG
  with $n_{\text{leaves}}$ \texttt{x}-leaves plus $n_{\text{leaves}}$
  \texttt{y}-leaves at the base, $n_{\text{compound}}$ compound terms of
  depth $d$ built from uniformly-chosen function symbols, and
  $n_{\text{merges}}$ random initial equalities, each pairing one
  \texttt{x}-leaf with one \texttt{y}-leaf.
...
While both \parentalgo and \filteralgo achieve similar parallelization on larger
...
round wastes work proportional to the entire term set for very few useful unions.
