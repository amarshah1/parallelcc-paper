...
Essentially, this setup divides the set of leaves into two groups and merges a
random subset of them across the groups. We stratify our workloads into
different size classes, which are shown in \autoref{tab:workloads}.
...
benchmark and the performance of our algorithms, we design a synthetic benchmark
suite that allows us to dial up these parameters.

Our benchmark suite is parameterized by a width parameter $k$ and a $g$-tree fan-in
...
The width parameter $k$ controls how many merges occur at the first congruence-closure
...
performance within this benchmark family. This ratio is roughly constant across thread counts, ranging from
...
congruence closure implementations on these gates. We use all 10 IWLS'22
miters, and from the HWMCC'12 set we pick the twelve hardest instances by
sequential congruence closure time, giving 22 files in total.
...
workloads. However, unlike on the random and synthetic benchmarks, the single-thread
...
on \texttt{h5m-n04-ands.gates}---the file with the largest single-thread
advantage over sequential---up to $32.3\times$ the sequential runtime on
...
\begin{figure}[H]
  ...
  \caption{Ratio of sequential baseline to \filteralgo runtime at $T{=}1$ vs.\ the smallest fraction of dirty terms across rounds.}
