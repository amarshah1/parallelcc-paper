...
\begin{itemize}
  ...
  self-speedup at $T{=}32$ relative to its $T{=}1$ baseline, $36.8\times$ over
  the sequential baseline), but begin to
  ...
  We discuss integration prospects in \autoref{sec:disc:future}.
\end{itemize}
...
theory solvers that share the e-graph. Even on pure equality with uninterpreted functions (EUF)
...
% this concurrent bag for new merges to perform. We found that this asynchronous
% variant never outperformed the synchronous version in any regime we measured.
