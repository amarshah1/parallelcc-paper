...
add_text(s, 'dethroned roots\n→ next Work', bx1 + BW / 2 + 0.12, 4.05, 2.2, 0.7, size=12, color=GRAY)
...
s = new_slide('ParentCC on the example: round 2, merge', notes=('One union; m2 is dethroned.'))
draw_egraph(s, classes=LEAF_CLASSES + [A_CLASS, X_CLASS, M_CLASS], highlight=['m1', 'm2'])
...
code = [
    ...
    ('Work ← Work ∪ (pre-merge roots of g) \\ {rep}', 3),
    ('', 0),
...
s = new_slide('Why ParentCC is correct', notes=(
    ...
    'term is dethroned at most once. Soundness by the invariant that union-find classes '
...
blocks = [
    ...
    ('Termination',
     ['a term enters Work only when dethroned', 'each term dethroned at most once',
      'at most |T| − 1 entries over the whole run']),
    ('Soundness',
